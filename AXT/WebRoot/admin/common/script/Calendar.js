...
L_calendar.prototype={
	_VersionInfo:"Version:3.0&#1;作者: Sam.y.zhang",
	Moveable:true,
	NewName:"",
	insertId:"",
	ClickObject:null,
	...
	Today:function()  //Today Button
  	{
    	...
    	if(this.L_TheMonth>0 && this.L_TheMonth <10) this.L_TheMonth = "0"+this.L_TheMonth;
    	if(today>0 && today <10) today = "0"+today;
    	if(this.InputObject){
	...
	SetDay:function (yy,mm)   //主要的写程序**********
	{
		...
		if(window.top.location.href!=window.location.href){
			//3月18号修改
			L_DateLayer_Parent=window;
		}
	/*	if(window.top.location.href!=window.location.href){
			alert(window.top);
			for(var i_f=0;i_f<window.top.frames.length;i_f++){
				alert(window.top.frames[i_f]);
		  		if(window.top.frames[i_f].location.href==window.location.href)
		  		{
		 			L_DateLayer_Parent=window.top.ContentIframe.frames[i_f];
		 			alert(L_DateLayer_Parent+'		sd');
